...
export function useMuseumApi() {
  ...
  async function searchImageFromUri(
    ...
    // If it's a remote URL, download it first to local cache
    if (photoUri.startsWith("http://") || photoUri.startsWith("https://")) {
      try {
        const filename = `temp_search_${Date.now()}.jpg`;
        const downloadPath = `${cacheDirectory}${filename}`;
        const downloadResult = await downloadAsync(photoUri, downloadPath);
